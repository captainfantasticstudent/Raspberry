...
while(end == 0):
    while(ser.inWaiting()):
        ...
        if toMe == 2 and (confirmFlag[count] == 1 or confirmFlagSTART == 1):
            if licznikUART < 7:
                ...
                if licznikUART == 7:
                    if  (values[count][2].encode() == tabUART[4] and values[count][1].encode() == tabUART[3] and values[count][0].encode() == tabUART[2]):
                        print(tabUART)
    ...
    if (czas - czas2 >= 0.5):
        ...
        if (((SQLread('START', 'Bb') == 1 and SQLread('GOTOWE', 'Bb') == 1) or resendSTART == 1) and confirmFlag[count] == 0):
